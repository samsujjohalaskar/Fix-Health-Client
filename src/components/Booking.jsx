...
const Booking = () => {
    ...
    const handleFindPhysiotherapist = () => {
        ...
        navigate(`/${formData.city.toLowerCase()}`);
        setStep(1);
        setFormData({
            name: '',
            phoneNumber: '',
                ...
                {(step === 2 || step === 3 || step === 4 || step === 5) &&
                    (<div div style={{ background: '', padding: '10px', marginBottom: '10px' }}>
                        {Array.from({ length: 5 }, (_, index) => (
                            <div key={index} style={{ width: '20px', height: '20px', background: step > index ? '#27374D' : '#9DB2BF', borderRadius: '50%', display: 'inline-block', margin: '0 5px' }}></div>
                        ))}
                            ...
                            <div className="bookings-label-inputs">
                                {[1, 2, 3, 4, 5].map((value) => (
                                    <label key={value} className='booking-inputs-label'>
                                        <input
                                            className="bookings-label-inputs-boxes"
                                            type="checkbox"
                                            name="painIntensity"
                                            value={value}
